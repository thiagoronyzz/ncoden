...
const say=H.msg(root,"Distribua <b>6 trabalhadores</b>: fazenda (+4 comida) · mina (+3 ouro) · recruta (+1 soldado). Ataques nos turnos 4, 8 e 12!");
const box=H.el("div","g-col",null,root);
function paint(){
  hud.set("tn",turn+"/12");hud.set("cm",food);hud.set("ou",gold);hud.set("ex",sol);
  box.innerHTML="";
  const rows=[["f","Fazenda"],["m","Mina"],["r","Recrutas"]];
  rows.forEach(([k,nm])=>{
...
H.btn(root,"Passar turno",()=>{
  ...
  if(turn===4||turn===8||turn===12){
    ...
    if(sol>=atk){const lost=Math.ceil(atk/2);sol-=lost;msg+="Ataque ("+atk+") repelido! −"+lost+" soldados. ";}
