...
let p=0,lock=false,bulbs=[],over=false,moves=0;
...
function build(){
  const P=PANELS[p];
  if(!P){if(!over){over=true;H.done({win:true,score:sc+80,title:"Mural iluminado!",sub:"3 painéis acesos com lógica."});}return;}
...
function paint(){
  ...
  PANELS[p].sw.forEach((s,i)=>{
    ...
    btn.addEventListener("click",()=>{
      ...
      if(bulbs.every(Boolean)&&!lock){
        lock=true;
        sc+=120;H.score(sc);hud.set("sc",sc);H.sfx("ok");
        p++;
        if(p>=PANELS.length){over=true;lock=false;return H.done({win:true,score:sc+80,title:"Mural iluminado!",sub:"3 painéis acesos com lógica."});}
        say("Painel aceso! +120. Próximo: grupos diagonais/blocos.");
        H.after(600,()=>{lock=false;build();});
      }
    });
  });
}
