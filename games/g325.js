...
function status(){
 ...
 st.innerHTML='Dia '+day+'/15 · Ações: '+act+'<br> '+thirst.toFixed(0)+' · '+hunger.toFixed(0)+' · '+shade+'%';
 paintBtns();
}
function paintBtns(){
 brow.innerHTML='';
 ...
 H.btn(brow,'Improvisar sombra (+25%)',()=>{if(act<=0||over)return;shade=Math.min(100,shade+25);act--;H.sfx('tick');after();},false);
}
function after(){
 ...
 thirst-=sun/2;
 if(Math.random()<.3){thirst=Math.min(100,thirst+30);say('Chuva! +30 sede.');}
