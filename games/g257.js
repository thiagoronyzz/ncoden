...
const SKL={investida:'',escudo:'',dreno:'',cura:'♥'};
...
function hitMinion(w,i,dmg){
  const m=board[w][i];
  if(!m)return false;
  if(m.sh){m.sh=false;say2(m.n+' bloqueou com !');return false;}
  m.hp-=dmg;
  if(m.hp<=0){
    board[w].splice(i,1);
    if(!w)kills++;
...
function castSpell(w,card,target){
  ...
  if(card.id===0){
    ...
    hp[1-w]-=2;say2('Meteoros: 2 em tudo!');
  }
  H.sfx('ok');
}
...
function cardLabel(c){
  if(c.t==='c'){const p=POOL[c.id];return'<b>'+p[0]+'</b> '+p[1]+'<br>'+p[2]+'♥'+p[3]+(p[4]?' '+SKL[p[4]]:'');}
  const s=SPELLS[c.id];return'<b>'+s[0]+'</b> '+s[1]+'<br><span style="font-size:11px">'+s[2]+'</span>';
}
function minLabel(m){
  return'<b>'+m.n+'</b><br>ATQ '+m.atk+' · HP '+m.hp+(m.sk?' '+SKL[m.sk]:'')+(m.can?'':' (dormindo)');
}
function paint(){
  if(over)return;
  ehero.innerHTML='<b>CPU</b> · ♥ '+Math.max(0,hp[1])+' · '+mana[1]+'/'+maxm[1]+' · deck '+deck[1].length;
  eboard.innerHTML='';
  board[1].forEach((m,i)=>{
  ...
  phero.innerHTML='<b>Você</b> · ♥ '+Math.max(0,hp[0])+' · '+mana[0]+'/'+maxm[0]+' · deck '+deck[0].length;
  hrow.innerHTML='';
  hand[0].forEach((c,hi)=>{
    const cost=c.t==='c'?POOL[c.id][1]:SPELLS[c.id][1];
  ...
  brow.innerHTML='';
  if(turn===0)H.btn(brow,'Encerrar turno',()=>{if(!over&&turn===0){startTurn(1);}},true);
}
