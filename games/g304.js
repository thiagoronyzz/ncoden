...
const say=H.msg(root,'Corra até o fim! ↑/Espaço pula os muros baixos, ↓ desliza sob as barras altas. 3 erros = fim!');
const o=H.cvs(root,560,340),x=o.x;
const kb=H.keys(),dn={};kb.on((c,d)=>{if(!d)return;if(c==='Space'||c==='ArrowUp'||c==='KeyW')jump();});
H.btn(root,'↑ PULAR',jump,false);
H.btn(root,'↓ DESLIZAR',()=>{if(!over&&ground){slide=.5;H.sfx('tick');}},false);
...
H.loop(dt=>{
 ...
 x.fillText(slide>0?'i:sled':'i:run',120,272+py);
});
}});
