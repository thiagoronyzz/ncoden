...
const say=H.msg(root,'↑ acelera, ←→ inclinam! Nas curvas, incline para o lado indicado ou derrapa para fora. Complete a pista!');
...
function gameOver(win){over=true;H.sfx(win?"win":"lose");const sc=win?Math.max(250,800-(t|0)*8):px/32|0;H.score(sc);
H.done(win?{win:true,score:sc,title:'Volta perfeita!',sub:'Em '+t.toFixed(1)+'s.'}:{win:false,score:sc,title:'Derrapou para fora!',sub:'Incline com força nas curvas sinalizadas!'});}
H.loop(dt=>{
 if(over)return;t+=dt;
 ...
 CURVES.forEach(q=>{
  ...
  if(qx>-260&&qx<620){
   ...
   x.fillText(q.dir>0?'→':'←',qx,200);
  }
 });
 x.save();x.translate(140,190);x.rotate(lean*.5);
 x.font='36px system-ui';x.textAlign='center';x.fillText('',0,12);x.restore();
 ...
 if(c){x.fillStyle='#D94E34';x.font='bold 18px system-ui';x.fillText('CURVA '+(c.dir>0?'→':'←')+'!',200,60);}
});
}});
