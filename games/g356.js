...
H.loop(dt=>{
 ...
 for(let i=0;i<16;i++){const cx=(i*211-cam%211+211)%571-10;x.fillText('i:coral',cx,340);}
 LIFE.forEach(s=>{
  const sx=s.x-cam;
  if(sx>-40&&sx<600){
   x.font='34px system-ui';x.fillText(s.k,sx,s.y+Math.sin(t*2+s.x)*6);
   if(s.got){x.font='18px system-ui';x.fillText('i:check',sx,s.y-28);}
  }
 });
 x.font='30px system-ui';x.fillText('i:diver',px-cam,py+10);
});
}});
