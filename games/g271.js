...
const say=H.msg(root,'Cave até 100%! Cavar faz barulho — pare quando o guarda passar (luz vermelha)! Cavar devagar é sempre seguro.');
const o=H.cvs(root,460,340),x=o.x;
H.btn(root,'Cavar rápido (+8, barulhento)',()=>{
 if(over)return;
 prog+=8;H.sfx('tick');
...
H.done(win?{win:true,score:sc,title:'Liberdade!',sub:'Túnel pronto em '+(150-Math.ceil(time))+'s.'}:{win:false,score:sc,title:'Pego no flagra!',sub:'Túnel em '+(prog|0)+'%. Não cave sob a luz vermelha!'});}
H.loop(dt=>{
 ...
 x.font='30px system-ui';x.fillText('i:brick',415,245);x.fillText('i:pick',30+400*Math.min(1,prog/100),205);
 x.fillStyle='#fff';x.font='bold 14px system-ui';x.textAlign='left';
 x.fillText('Túnel '+Math.min(100,prog|0)+'% · Desconfiança '+(caught|0)+'% · '+Math.ceil(time)+'s',14,300);
