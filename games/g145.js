...
const say=H.msg(root,"<b>Repor</b> enche a gôndola (evita fuga). <b> Passar compra</b> atende 1 da fila (2s cada). joga fora o <b>vencido</b> antes que mancha a loja!");
...
function paint(){
  ...
  sbox.innerHTML="";
  shelf.forEach((s,i)=>{
    const b=H.el("button","g-chip"+(exp[i]>0?" bad":s<=1?" hot":""),NM[i]+" "+s+(exp[i]>0?""+exp[i]+" venc!":""),sbox);
    b.style.cursor="pointer";
    b.addEventListener("click",()=>{
      if(over)return;
      if(exp[i]>0){exp[i]=0;H.sfx("ok");say("Vencidos descartados!");}
      else{shelf[i]=Math.min(9,shelf[i]+3);H.sfx("tick");}
      paint();
    });
  });
  qbox.innerHTML="fila: "+queue.length+" esperando"+(scan>0?" · passando compra…":"");
}
paint();
