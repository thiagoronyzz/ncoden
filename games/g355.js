...
const say=H.msg(root,'Atravesse até a caverna e pegue 4 fósseis ! Fendas azuis = volte ao início. Frio sobe parado — mexa-se!');
...
H.loop(dt=>{
 ...
 if(CREV.some(c=>Math.abs(px-c.x)<c.w/2+6&&Math.abs(py-c.y)<90)){px=40;py=300;tx=px;ty=py;H.sfx('bad');say('Fenda! De volta ao início.');}
 FOS.forEach(f=>{
 ...
 FOS.forEach(f=>{if(!f.got){x.font='22px system-ui';x.textAlign='center';x.fillText('i:bone',f.x,f.y);}});
 x.font='30px system-ui';x.fillText('i:hole',530,200);
