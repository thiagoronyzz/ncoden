...
H.loop(dt=>{
 ...
 CARS.forEach(c=>{
  ...
  x.font='30px system-ui';x.textAlign='center';x.fillText(c.hit?'i:burst':'i:car',c.x,cy+10);
 });
 COINS.forEach(c=>{
  const cy=c.y+dist;
  if(!c.got&&cy>-20&&cy<540){x.font='20px system-ui';x.fillText('i:coin',c.x,cy+7);}
 });
 x.font='32px system-ui';x.fillText('i:cart',px,442);
});
}});
