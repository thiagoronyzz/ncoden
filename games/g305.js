...
const say=H.msg(root,'←→ movem E equilibram! Desvie dos pedestres e não deixe o equilíbrio zerar. Meta: 600 pontos!');
...
H.loop(dt=>{
 ...
 PED.forEach(p=>{
  ...
  x.font='24px system-ui';x.textAlign='center';x.fillText(p.hit?'i:burst':'i:walk',p.x,py+8);
 });
 x.save();x.translate(px,430);x.rotate(bal/300);
 x.font='32px system-ui';x.textAlign='center';x.fillText('i:scooter',0,10);x.restore();
 ...
 x.fillText((score|0)+' pts · ♥'.repeat(1)+' '+lives+' · meta 600',12,50);
});
}});
