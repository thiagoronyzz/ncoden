...
const say=H.msg(root,"Carro na bomba? Clique para <b> abastecer</b> (3s). = clique 2ª vez para o <b>óleo</b>. clientes compram lanche sozinhos (se houver)!");
...
function paint(){
  ...
  pumps.forEach((p,i)=>{
    ...
    else b.innerHTML=""+(p.fuel>0?"abastecendo "+Math.ceil(p.fuel)+"s":p.oil?"ÓLEO? clique!":"✔ pronto, clique!")+"<br>"+Math.ceil(p.p);
...
function act(i){
  ...
  if(p.oil){p.oil=false;cash+=6;H.sfx("ok");say("Óleo verificado! +$6");paint();return;}
