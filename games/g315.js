...
let debris=[];
for(let i=0;i<14;i++)debris.push({x:Math.random()*440+20,y:80+Math.random()*280,k:Math.random()<.6?'i:log':'i:rope',got:false,ph:Math.random()*7});
H.onTap(o,(px,py)=>{
 if(over||phase!=='build')return;
 debris.forEach(d=>{
  if(!d.got&&Math.hypot(px-d.x,py-d.y)<26){
   d.got=true;
   if(d.k==='i:log')wood++;else rope++;
   H.sfx('tick');
...
H.loop(dt=>{
 ...
 if(phase==='build'){
  ...
  x.fillText('i:log'+wood+'/6 '+rope+'/4',240,30);
 }else{
  x.font='44px system-ui';x.textAlign='center';
  x.fillText('i:canoe',240+Math.sin(t*3)*8,220+Math.sin(t*5)*6);
  x.fillStyle='#fff';x.font='bold 15px system-ui';x.textAlign='left';
  x.fillText('i:wave'+Math.ceil(storm)+'s · água '+(water|0)+'% · ♥ '+(hp|0),14,30);
