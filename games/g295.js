...
const say=H.msg(root,'Plane até a zona de pouso ■! Suba nas térmicas ↑ (colunas quentes), afunde fora delas. ←→ controlam a velocidade!');
const o=H.cvs(root,560,400),x=o.x;
const kb=H.keys(),dn={};kb.on((c,d)=>{dn[c]=d;});
function gameOver(win,why){over=true;H.sfx(win?"win":"lose");const sc=win?Math.max(200,800-(t|0)*5):dist/64|0;H.score(sc);
...
H.loop(dt=>{
 ...
 if(alt<=0){
  ...
  else gameOver(false,'Caiu a '+((LZ.x-px)|0)+'m da zona. Use as térmicas ↑!');
  return;
 }
 ...
 x.fillText('Alt '+Math.max(0,alt|0)+'m  '+(px/6550*100|0)+'%'+(inTh?'↑ TÉRMICA!':''),12,26);
});
}});
