...
const say=H.msg(root,'↓ agacha na descida (mais velocidade)! Na rampa, ←→ ajustam a inclinação do voo. Pouse reto! Melhor de 3 saltos.');
...
function gameOver(){over=true;const win=best>=70;H.sfx(win?"win":"lose");H.score(best|0);
...
function nextJump(crash,d){
 if(crash){say('Queda! Pouse com o corpo reto (incline ←→).');}
...
H.loop(dt=>{
 ...
 x.font='26px system-ui';x.textAlign='center';x.fillText('i:ski',0,8);x.restore();
 x.fillStyle='#181816';x.font='bold 15px system-ui';x.textAlign='left';
 x.fillText(phase==='down'?'DESCIDA — segure ↓! Vel '+sp.toFixed(0):'VOO! Incline ←→',12,28);
